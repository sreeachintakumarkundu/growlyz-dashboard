...
export default function SignupPage() {
  ...
  const passwordStrength = (() => {
    ...
    if (password.length < 6) return { label: 'Too short', color: 'text-red-400', bar: 'bg-red-500', width: '33%' };
    if (password.length < 10) return { label: 'Fair', color: 'text-yellow-400', bar: 'bg-yellow-500', width: '66%' };
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch('/api/auth/signup', {
        ...
        body: JSON.stringify({ phone: phone.trim(), password, name: name.trim(), email: email.trim(), securityQuestion, securityAnswer: securityAnswer.trim() }),
      });
    ...
    >
      <div className="w-full max-w-md animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-1 text-sm">Start tracking your ad performance</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-700/40 border border-slate-600 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 transition-all text-sm"
              />
              <p className="text-xs text-slate-500 mt-1">Used for password recovery</p>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  autoComplete="new-password"
                  className={`w-full bg-slate-700/40 border rounded-xl px-4 py-3 pr-11 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 transition-all text-sm ${
            ...
            <div>
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className={`w-full bg-slate-700/40 border rounded-xl px-4 py-3 text-left text-sm flex items-center justify-between transition-all focus:outline-none focus:ring-2 focus:ring-orange-500/50 ${showQuestionDropdown ? 'border-orange-500 ring-2 ring-orange-500/50' : 'border-slate-600'}`}
                >
                  <span className={securityQuestion ? 'text-white' : 'text-slate-500'}>
                    {securityQuestion || 'Select a question...'}
                  </span>
                  <svg className={`w-4 h-4 text-slate-400 flex-shrink-0 ml-2 transition-transform ${showQuestionDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="absolute z-50 w-full mt-1 bg-slate-800 border border-slate-600 rounded-xl shadow-xl overflow-hidden">
                    {SECURITY_QUESTIONS.map(q => (
                      <button
                        key={q}
                        type="button"
                        onClick={() => { setSecurityQuestion(q); setShowQuestionDropdown(false); }}
                        className={`w-full text-left px-4 py-3 text-sm transition-colors hover:bg-slate-700 ${securityQuestion === q ? 'text-orange-400 bg-orange-500/10' : 'text-slate-300'}`}
                      >
                        {q}
                      </button>
                    ))}
                ...
                <input type="text" value={securityQuestion} required readOnly className="sr-only" tabIndex={-1} />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">
              ...
              <input
                ...
                className="w-full bg-slate-700/40 border border-slate-600 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 transition-all text-sm"
              />
              <p className="text-xs text-slate-500 mt-1">Used to verify your identity if you forget your password</p>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 rounded-xl font-semibold text-white text-sm transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg hover:shadow-orange-500/25 hover:-translate-y-0.5 active:translate-y-0 mt-2"
